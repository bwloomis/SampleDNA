...
namespace WindowsFormsApplication2importableCobb.AWSManagers
{
    class clsS3Manager
    {
        ...
        public clsS3Manager()
        {
            ...
            logger.Trace("INFO: Initializing S3 client for profile {0}", appConfig["AWSProfileName"]);  // TODO: convert to AppSettinngs[]

            bucketName = appConfig["InitialS3Bucket"];
            if (string.IsNullOrEmpty(bucketName))
            {
                logger.Trace("INFO: there is no S3 bucket - aborting");
                throw new Exception("No S3 target bucket defined");
            }
            else
            {
                logger.Trace("INFO: Setting initial S3 bucket to {0}", bucketName);
            }
        ...
        public string UploadFile(string sourceFilePath, string destName)
        {
            ...
            catch (AmazonS3Exception amazonS3Exception)
            {
                ...
                {
                    logger.Error("ERROR: An error occurred with the message '{0}' when writing an object", amazonS3Exception.Message);
                }
            }
        ...
        public bool CreateBucket(string b)
        {
            ...
            catch (AmazonS3Exception amazonS3Exception)
            {
                ...
                {
                    logger.Error("ERROR: An error occurred with the message '{0}' and error code {1} when writing an object", amazonS3Exception.Message, amazonS3Exception.ErrorCode);
                    return false;
                }
            }
        ...
        public void ListBuckets()
        {
            try
            {
                ...
                foreach (S3Bucket bucket in response.Buckets)
                {
                    logger.Trace("INFO: Bucket listing found {0}", bucket.BucketName);
                }
            }
            catch (AmazonS3Exception amazonS3Exception)
            {
